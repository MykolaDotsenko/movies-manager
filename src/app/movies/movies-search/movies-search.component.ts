...
export class MoviesSearchComponent implements OnInit {
  ...
  genresServise = inject(GenresService);
  moviesServise = inject(MoviesService);
  pagination: PaginationDTO = { page: 1, recordsPerPage: 5 };
  totalRecordsCount!: number;

  ngOnInit(): void {
    this.genresServise.getAll().subscribe((genres) => {
      this.genres = genres;
  ...
  filterMovies(values: MoviesSearchDTO) {
    ...
    this.moviesServise.filter(values).subscribe((response) => {
